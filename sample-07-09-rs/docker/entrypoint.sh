#!/usr/bin/env sh
LOG_DIR=/webauthn/log
LOG_FILE=${LOG_DIR}/webauthn.log
LOG_SIZE=10M
LOG_NUM=10

LOGGING=${LOG_TO_FILE:-false}
USER=${HOST_USER:-webauthn}
...
EOF
  ...
  cat > /etc/logrotate.d/webauthn.conf << EOF
${LOG_FILE} {
    dateext
    daily
    missingok
...
function setup_ubuntu () {
  ...
  if [ ! $(id ${USER}) ]; then
    echo "webauthn: Create user ${USER} with ${USER_ID}:${GROUP_ID}"
...
if [ $(whoami) != "root" -o $(id -u) -ne 0 -a $(id -g) -ne 0 ]; then
  ...
  echo "If you want to run 'webauthn-sample' with a specific user, use HOST_USER, HOST_UID and HOST_GID environment variables."
  exit 1
fi
...
chown -R ${USER_ID}:${USER_ID} /webauthn

# run webauthn sample server
echo "Start with user: ${USER} (${USER_ID}:${GROUP_ID})"
if "${LOGGING}"; then
  echo "Start with writing log file"
  gosu ${USER} sh -c "/webauthn/run.sh 2>&1 | tee ${LOG_FILE}"
else
  echo "Start without writing log file"
  gosu ${USER} sh -c "/webauthn/run.sh 2>&1"
fi
